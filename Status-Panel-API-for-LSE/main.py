...
DEFAULT_CONFIG = {
    ...
    "console_log": True,         # 是否启用控制台日志 [新增配置]
...
system_data = {
    ...
    "system_type": "",        # 操作系统类型 (如 "Windows 11", "Debian 12")
    "system_kernel": "",      # 内核版本 (如 "NT 10.0.22621", "5.15.0-107-generic")
    "gpu_name": "N/A",
    "gpu_usage": 0.0,
    # 新增磁盘信息
...
def get_aggregated_status():
    ...
    try:
        with system_data_lock:
            system_status = {
                ...
                "system_type": system_data["system_type"],    # 操作系统类型
                "system_kernel": system_data["system_kernel"],  # 内核版本
    ...
    # 如果游戏监控开启，则包含游戏数据
    if config.get("game_monitor_enabled", True):
        try:
            with game_data_lock:
                ...
                if game_server_data["status"] == "offline":
                    response_data["code"] = STATUS_CODES["GAME_SERVER_DOWN"]
                
                # 添加游戏状态
                response_data["game_server_status"] = game_status
        except Exception as e:
